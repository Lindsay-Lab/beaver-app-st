...
def S2_PixelExtraction_Export(Dam_Collection, S2, Hydro, selected_datasets):
    def extract_pixels(box):
        ...
        NHD_kernel = NHD_paint.focal_max(kernelType="circle", radius=3, units="meters", iterations=8)
        ...
        def add_band(image):
            ...
            Full_image = (
                image.addBands(NHD_raster.rename("NHD_Hydro"))
                .addBands(additional_bands)
                .set(
                    {
                        "First_id": ee.String(damId)
                        .cat("_")
                        .cat(DamStatus)
                        .cat("_S2id:_")
                        .cat(index)
